...
const Hero = () => {
  return (
    <section className="relative w-full h-screen mx-auto flex flex-col justify-center overflow-hidden">  
      <div
        className={`${styles.paddingX} absolute inset-0 top-[120px] max-w-[80%] mx-auto flex flex-col items-center`}
      >
    <div className="w-full  flex flex-col">
  <h1 className={`${styles.heroHeadText} text-white`}>
    Hi, I'm <span className="text-yellow-300">Davis 🤓 ,<br></br> Software Developer 👨🏽‍💻</span>
  </h1>
  ...
  <div className="hidden md:block flex-1">
    <ul>
      {socialLinks.map((link) => (
        <li key={link.name} className="flex justify-center mt-8">
